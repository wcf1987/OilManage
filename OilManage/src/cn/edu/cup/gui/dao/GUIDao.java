...
//		tx.commit();
	...
	public List<PointProper> getPointProperList(int page, int rows,
		 ...
		 SQLQuery q = session.createSQLQuery("select t2.id,t2.point_type,t2.par_name,t2.par_display,t3.CName,t3.Symbol,t3.ID mess_id from t_guipointproper t2,t_measure t3 WHERE t2.par_messID=t3.ID  order by t1."+sidx+" "+sord);
	...
	public int addProper(String point_type,String par_name, String par_display,
			 int measure_id) {
		Query q = session.createSQLQuery("insert into t_guipointproper (point_type,par_name,par_display,measure_id) values (?,?,?,?)");
